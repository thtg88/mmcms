...
class MakeContentModelDestroyRequest
{
    ...
    public function handle(ContentModelStored $event)
    {
        ...
        if(!class_exists($request_name))
        {
            ...
            if(file_exists(app_path('Http/Requests/'.$request_name.'.php')))
            {
                ...
                {
                    $replace_content = $this->getContentModelDestroyRequestAuthorizeMethodContent();

                    $file_content = str_replace('return false;', $replace_content, $file_content);

                    $replace_content = $this->getContentModelDestroyRequestConstructor($model_name);

                    $file_content = str_replace("class ".$request_name." extends FormRequest\n{\n", $replace_content, $file_content);

                    $replace_content = $this->getContentModelDestroyRequestImports($model_name);
    ...
    /**
     * Returns imports for the destroy request from a given model name.
     *
     * @param   string  $model_name
     * @return  string
     */
    private function getContentModelDestroyRequestImports($model_name)
    {
        $content = '';
    ...
    /**
     * Returns the constructor for the destroy request from a given model name.
     *
     * @param   string  $model_name
     * @return  string
     */
    private function getContentModelDestroyRequestConstructor($model_name)
    {
        $content = '';
    ...
    /**
     * Returns the authorize method content for the destroy request from a given model name.
     *
     * @return  string
     */
    private function getContentModelDestroyRequestAuthorizeMethodContent()
    {
        $content = '';
